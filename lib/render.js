...
function checkCurrentPath(prefix) {
    return function (path) {
        if (!path || typeof path === 'string') {
            return false;
        } else {
            return path.startsWith(baseroot(prefix));
        }
    }
}
...
async function renderPage(post) {
    const url = `${post.s || post.postName}`;
    const html = await render(
        path.join(THEME_DIR, 'page.pug'),
        inject(
            env({ page: true }),
            { post: _.assign(post, { content: await markdown(post.content, url, post.resourcesDir, { sanitize: false }) }) }
        )
    );
    const generatePath = `${OUTPUT_DIR}/${url}`;
    await fs.mkdirs(generatePath);
    await Promise.all([
